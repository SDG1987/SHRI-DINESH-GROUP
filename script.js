...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', async function (e) {
            ...
            const payload = {
                ...
                service_interest: formData.get('service') || 'General Inquiry',
                message: formData.get('message')
            };

            try {
                const response = await fetch('https://hrms.shridinesh.com/api/method/sdg.apis.contact.submit_contact', {
                    method: 'POST',
                    headers: {
    ...
    if (footerForm) {
        footerForm.addEventListener('submit', async function (e) {
            ...
            const textInputs = footerForm.querySelectorAll('input[type="text"]');
            const craneInput = textInputs[0];
            const locInput = textInputs[1];

            const payload = {
                full_name: 'Footer Web Inquiry',
                email: emailInput.value,
                service_interest: craneInput.value,
                message: `Requirement: ${craneInput.value} | Location: ${locInput.value}`
            };
